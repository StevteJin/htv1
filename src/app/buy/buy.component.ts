...
export class BuyComponent implements DoCheck, OnDestroy, OnInit {
    ...
    */
    ...
    buy() {
        ...
        } else if (this.stockHQ.priceDownlimit > 0 && this.appointPrice < parseFloat(this.stockHQ.priceDownlimit).toFixed(2)) {
            this.data.ErrorMsg('委托价格不能低于跌停价');
        } else if (this.stockHQ.priceUplimit > 0 && this.appointPrice > parseFloat(this.stockHQ.priceUplimit).toFixed(2)) {
            this.data.ErrorMsg('委托价格不能高于涨停价');
        } else if (parseInt(this.appointCnt, 0) !== this.appointCnt) {
            this.data.ErrorMsg(`${this.text}数量必须是整数`);
        } else if (this.appointCnt < 200 && /^688[0-9]{3}$/.test(this.data.stockHQ.stockCode)) {
            this.data.ErrorMsg(this.text + '数量必须大于等于200股');
        } else if (this.appointCnt % 100 !== 0 && !/^688[0-9]{3}$/.test(this.data.stockHQ.stockCode)) {
        ...
        } else if (/^688[0-9]{3}$/.test(this.data.stockHQ.stockCode)) {
            this.submitAlert = this.data.show;
